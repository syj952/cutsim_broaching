...
int Machine::checkLimit(double x, double y, double z, double a, double b, double c) {
    ...
    if (!isnan(max_a_limit) && (a > max_a_limit)) result |= MAX_A_LIMIT;
    else if (!isnan(min_a_limit) && (a < min_a_limit)) result |= MIN_A_LIMIT;
    if (!isnan(max_b_limit) && (b > max_b_limit)) result |= MAX_B_LIMIT;
    else if (!isnan(min_b_limit) && (b < min_b_limit)) result |= MIN_B_LIMIT;
    if (!isnan(max_c_limit) && (c > max_c_limit)) result |= MAX_C_LIMIT;
    else if (!isnan(min_c_limit) && (c < min_c_limit)) result |= MIN_C_LIMIT;
