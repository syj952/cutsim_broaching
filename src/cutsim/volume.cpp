...
#define GRID    (1.0)
    ...
    //************* AptCutterVolume **************/
    AptCutterVolume::AptCutterVolume() {
        type = APT_VOLUME;
        cuttertype = APT;
        radius = 0.0;
        length = 0.0;
    ...
    void AptCutterVolume::calcBB() {
        bb.clear();

    }

    double AptCutterVolume::get_blade_angle_1(double z)const {
    ...
    double AptCutterVolume::get_blade_angle_2(double z)const {
        return tool_angle + tan(a_1) * H_1 / (tan(b_1) + r_1 - H_1 * tan(b_1)) + tan(a_2) * (z - H_1 - z_start) / (tan(b_2) + r_2 - H_2 * tan(b_2));
    }

    double AptCutterVolume::dist(const GLVertex & p) const {
        GLVertex t = p - center;
    ...
    Cutting AptCutterVolume::dist_cd(const GLVertex & p) const {
        Cutting result = { 0.0, NO_COLLISION, 1 };
        result.f = dist(p);
        // 可根据需要扩展碰撞类型判定
        return result;
    ...
    std::vector<std::vector<std::vector<double>>>
        AptCutterVolume::convertTo3DVibrationVectors(const std::vector<std::vector<double>>&pre_vibration_vectors,
            int num_dofs,
    ...
    double AptCutterVolume::calculateCutThickness(const GLVertex & current_point, const GLVertex & prev_point1, const GLVertex & prev_point2,
    ...
    int  AptCutterVolume::readBladeAnglesFromFile(const std::string & filename, int blade_id) {
        // 确保blade_id在有效范围内
        if (blade_id < 0) {
    ...
    void AptCutterVolume::calculatePosition_balde() {
    ...
    void AptCutterVolume::calculateForceData() {
        // 清空所有力数据映射
        force_map.clear();
        ...
        for (const auto& pair : cut_h) {
            ...
            if (inside_index < static_cast<int>(current_gamma.size()) &&
                ...
                // 计算合力 (force_value)
                const double force_param_scale = 0.1;

                k_fc = force_param_scale * (
                    17587 - 29.54 * v_c
                    - 199.7 * current_gamma[inside_index]
                    - 572.45 * current_alpha[inside_index]
                    - 469307 * current_cut_h[inside_index]
                    + 6411 * current_gamma[inside_index] * current_cut_h[inside_index]
                    + 21542 * current_alpha[inside_index] * current_cut_h[inside_index]
                    );
                k_fcn = force_param_scale * (
                    16476 + 65 * v_c
                    - 331.1 * current_gamma[inside_index]
                    - 192.68 * current_alpha[inside_index]
                    - 445119 * current_cut_h[inside_index]
                    + 8808.7 * current_gamma[inside_index] * current_cut_h[inside_index]
                    );
    ...
    void AptCutterVolume::outputForceData(const std::string & output_dir) {
    ...
    void AptCutterVolume::calculateTotalForce() {
    ...
    void AptCutterVolume::getCurrentTotalForce(double* Fx, double* Fy, double* Fz) {
    ...
    void AptCutterVolume::outputTotalForceData(const std::string & output_dir) {
    ...
    void AptCutterVolume::calculateVibration() {
    ...
    void AptCutterVolume::updatestockVibrParams() {
        // 修改为基于模态的参数计算
    ...
    void AptCutterVolume::calculatestockVibration() {
        // 获取自由度数量和模态数量
    ...
    // 新增计算最大最小值函数
    void AptCutterVolume::calculateStraightness() {
        // 遍历外层映射（角度）
    ...
    void AptCutterVolume::outputStraightnessData(const std::string & output_dir) {
        // 收集所有存在的刀具ID和点索引（去重）
