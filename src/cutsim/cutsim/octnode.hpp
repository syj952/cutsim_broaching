...
public:
    ...
    CutState currentcutstate;//节点切削状态
    static const GLVertex* getDirection() { return direction; }
    ///added hust
    ...
    ///added hust
    void diff_blade(const Volume* vol);
    ...
    void force_setInside()    { prev_state = state; state = INSIDE;	   }
    void force_setOutside()   { prev_state = state; state = OUTSIDE;   }
    ...
    /// based on the f[]-values at the corners of this node, set the state to one of inside, outside, or undecided with limit.
    bool check_f_value_with_limit();
    bool check_complete_inside_outside();
    bool check_include_undecided();
    int  check_node_state();

    void nodeTransfer(GLVertex parallel, int flip_axis = 0, bool ignore_parts = false); // flip_axis, X_AXIS:1 Y_AXIS:2, Z_AXIS:3
    void force_delete_children();
