...
namespace cutsim {
    ...
    public:
        ...
        void cuda_diff(const Volume* vol, double max_depth) { cuda_diff(this->root, vol, max_depth); }
        void cuda_sum_stl(const StlVolume* vol, double max_depth) { cuda_sum_stl(this->root, vol, max_depth); }
        void blade_diff(const Volume* vol) { blade_diff(this->root, vol); }
        void broaching_cuda_blade_diff(broaching_AptCutterVolume* vol) { broaching_cuda_blade_diff(this->root, vol); };
        void milling_cuda_blade_diff(milling_AptCutterVolume* vol) { milling_cuda_blade_diff(this->root, vol); };
        void digitaltwin_milling_cuda_blade_diff(digitaltwin_AptCutterVolume* vol) { digitaltwin_milling_cuda_blade_diff(this->root, vol); };
    ...
    protected:
        ///added hust
        void cuda_diff(Octnode* current, const Volume* vol, double max_depth);
        void cuda_sum_stl(Octnode* current, const StlVolume* vol, double max_depth);
        void cuda_sum(Octnode* current, const Volume* vol, double max_depth);
        void blade_diff(Octnode* current, const Volume* vol);
        void broaching_cuda_blade_diff(Octnode* current, broaching_AptCutterVolume* vol);
        void milling_cuda_blade_diff(Octnode* current, milling_AptCutterVolume* vol);
        void digitaltwin_milling_cuda_blade_diff(Octnode* current, digitaltwin_AptCutterVolume* vol);
