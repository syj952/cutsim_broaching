...
public slots:
    ...
    void slotRequestMove() {
        ...
        if (cl->isMotion() ) {
            ...
            if (m == 0) {
                ...
                SpindleSpeed = cl->getStatus()->getSpindleSpeed();
...
#else
            emit signalToolPosition( pos.x, pos.y, pos.z, current_line, motionStatus, feed_rate,SpindleSpeed);
#endif
    ...
    /// clear bufferd cannon lines
    void clearCanonLine(void) {
        if (play_flag == false)
            emit signalProgress(0, 0, 0.0, true);
        play_flag = false;
        first = true;
...
#else
    void signalToolPosition( double x, double y, double z, int line, int mstatus, double feedrate ,double SpindleSpeed); // 3-axis.
#endif
