...
linearMotion::linearMotion(std::string canonL, machineStatus prevStatus): canonMotion(canonL,prevStatus) {
    ...
    //  endDir = (status.getEndPose().dir + status.getOrigin().dir) * (SIGN * CUTSIM_PI/180.0);
    endDir.x = (status.getEndPose().dir.x + status.getOrigin().dir.x) * (SIGN_A * CUTSIM_PI/180.0);
    endDir.y = (status.getEndPose().dir.y + status.getOrigin().dir.y) * (SIGN_B * CUTSIM_PI/180.0);
    endDir.z = (status.getEndPose().dir.z + status.getOrigin().dir.z) * (SIGN_C * CUTSIM_PI/180.0);
#endif
}
