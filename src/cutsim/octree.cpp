...
namespace cutsim {
    ...
    void Octree::cuda_blade_diff(Octnode* current, AptCutterVolume* vol) {
        // 使用CUDA版本
        //qDebug()<<"使用CUDA版本";    
...
#pragma omp critical
    ...
    void Octree::export_mesh_to_file(std::vector<GLVertex*>& normalvertices, std::vector<std::vector<int>>& boundaryFaces, std::vector<GLVertex*>& hanging_vertices, const std::string& filename)
...
for (const auto& face : boundaryFaces) {
    ...
    void Octree::clearVertexStates() {
        clearVertexStates(root);
        std::cout << "已清除所有节点顶点状态" << std::endl;
    }

    /// 递归清除节点顶点状态的实现
    void Octree::clearVertexStates(Octnode* current) {
        if (!current) return;
