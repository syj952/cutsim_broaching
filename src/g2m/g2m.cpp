    ...
    if ( file.endsWith(".ngc") ) {
        ...
        if ( fileHandle.open( QIODevice::ReadOnly | QIODevice::Text) ) {
            ...
            while ( !t.atEnd() ) {
                ...
                if (gline.startsWith("%"))
                    gline = "";
                // for Irregular code G1F***
                if (gline.startsWith("G1F")) {
                    gline.remove(0,2);
                    irregular_G1F = true;
                } else if (irregular_G1F == true) {
                    gline = QString("G1") + gline;
                    irregular_G1F = false;
                }
#endif
        ...
        filelist.file = file;
        filelist.lines = gcode_lines;
        filelist.position = total_gcode_lines;
        filelists.push_back(filelist);
        emit signalProgressFeature(QString("Interpret g-code"), total_gcode_lines, total_gcode_lines + gcode_lines);
        ...
        QProcess wc;
        bool wcFlag = false;
        wc.start(tr("wc -l %1").arg(file));
        if (wc.waitForReadyRead()) {
            QString str = wc.readAll();
            QStringList list = str.split(" ");
            emit signalProgressFeature(QString("Loading Canon-Lines"), 0, list[0].toInt());
            wcFlag = true;
        } else
            emit debugMessage ("Can't evoke wc!");
        ...
        while(std::getline(inFile, sLine)) {
            ...
            if (wcFlag == true)
                emit signalProgressValue(++lines);
        }
        ...
        if (toCanon.canReadLine()) {
            ...
            if (lineLength != -1 ) {
                ...
                if (cmt.contains("COMMENT(\"Gcode Line No.")) {
                    total_gcode_lines++;
                    emit signalProgressValue(total_gcode_lines);
                } else {
                    l += line;
                    foundEOF = processCanonLine(line); // line is a canon-line
