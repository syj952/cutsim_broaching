...
#define _vsprintf_s_l(theBuffer, theLocale, theFormat, theArgPtr) _vsprintf_l(theBuffer, theFormat, theLocale, theArgPtr)
...
#define _vsprintf_s_l(theBuffer, theLocale, theFormat, theArgPtr) vsprintf(theBuffer, theFormat, theArgPtr)
...
void Msg::ShowWarning(const OCCT_Utf8String & Message, bool Global)
{
	if (Global)
	{
		NCollection_Map<Messager *>::Iterator Itor(m_Msgrs);
...
void Msg::ShowInfo(bool Global, const char * theFormat, ...)
{
	...
	int result = _vsprintf_s_l(theBuffer, Standard_CLocaleSentry::GetCLocale(), theFormat, argp);
	va_end(argp);
	Msg::ShowInfo(theBuffer, Global);
}

void Msg::ShowWarning(bool Global, const char * theFormat, ...)
{
	char theBuffer[1024];
	SAVE_TL();
	va_list argp;
	...
	int result = _vsprintf_s_l(theBuffer, Standard_CLocaleSentry::GetCLocale(), theFormat, argp);
	va_end(argp);
	Msg::ShowWarning(theBuffer, Global);
}
