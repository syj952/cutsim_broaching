...
OCCT_ShapeList OCCT_ShapeImport::ImportModel(int partcutter)
{
	//partorcutter = partcutter;
	OCCT_ShapeList Shapes;
	//p_UI->partorcutter = partcutter;
	OCCT_Utf8StringList Paths = p_UI->getLoadPathList(partcutter);//打开资源管理器并获取路径列表
	p_UI->BeginWaitCursor();//设置等待光标
	...
	if (Ext == "STEP" || Ext == "STP")
	{
		...
		if (status == IFSelect_RetDone)
		{
			...
			if (OK)
				...
				p_UI->showError(OCCT_Utf8String("STEP加载转换失败"));
		}
		else
